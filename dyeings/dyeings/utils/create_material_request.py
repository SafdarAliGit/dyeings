...
@frappe.whitelist()
def create_material_request_chemicals(job_card_name):
...
@frappe.whitelist()
def create_material_request_dyes(job_card_name):
    """Create a Material Request (Material Transfer) from Job Card Dyeing child tables"""
    ...
    "Material Request",
    {
        "custom_job_card_dyeing_dyes": job_card.name,
        "docstatus": ("!=", 2),
        "material_request_type": "Material Transfer"  
    },
    ["name"],
    ...
    mr.company = job_card.company if hasattr(job_card, "company") else frappe.defaults.get_global_default("company")
    mr.transaction_date = job_card.date or frappe.utils.today()
    mr.custom_job_card_dyeing_dyes = job_card.name
